...
def _log_failed_prompt(extra_info):
    original_prompt = extra_info.get("original_prompt") if isinstance(extra_info, dict) else None
    if original_prompt is None:
        logger.warning("-" * 16 + "Failed Prompt unavailable" + "-" * 16)
        return

    logger.warning("-" * 16 + "Failed Prompt" + "-" * 16)
    logger.warning(str(original_prompt).replace("\n\n", "\n"))
...
def fuzzy_equal(actual: str, expected: str, tolerance: float = 1e-6, verbose=False) -> bool:
    ...
    if fuzzy_match_reasons and verbose:
        logger.info("😅 FUZZY MATCH - Outputs approximately equal:")
        logger.info(f"  Expected: {repr(original_expected)}")
        logger.info(f"  Actual:   {repr(original_actual)}")
        logger.info("  Reasons for fuzzy matching:")
...
def _compute_score(solution_str, ground_truth, extra_info, format_reward=0.0, answer_reward=1.0):
    ...
    if "pytest" in ground_truth or "functional" in ground_truth or "solution_file" in ground_truth:
        if "functional" in ground_truth:
            ...
            if CODER1_EXEC == "sandboxfusion":
                raise ValueError("SandboxFusion does not support executing full solution files, please switch to unsafe_local execution.")
            succ, output = code_exec(solution_code, solution=ground_truth["solution_file"])
        else:  # pytest
            if CODER1_EXEC == "sandboxfusion":
                raise ValueError("SandboxFusion does not support executing pytest, please switch to unsafe_local execution.")
            succ, output = code_exec(solution_code, pytest=ground_truth["pytest"])
        if not succ:
            logger.warning(
                "!" * 16
                + f"⚠️ Test Execution Failed"
                + "!" * 16
            )
            logger.warning(output[:_MAX_CHAR_DISPLAY])
            _log_failed_prompt(extra_info)
        ...
        with ThreadPoolExecutor(max_workers=min(8, len(stdin_list))) as executor:
            ...
            for future in as_completed(futures):
                ...
                if not succ or not fuzzy_equal(output.strip(), stdout.strip()):
                    ...
                    logger.warning(f"🔎Input: {repr(stdin)}")
                    logger.warning(f"✅Expected: {repr(stdout.strip())}")
                    logger.warning(
...
def compute_score(solution_str, ground_truth, extra_info, format_reward=0.0, answer_reward=1.0):
    ...
    reward_log = marker * 16 + "Reward Calculation" + marker * 16 + "\n" + reward_log + "\n" + marker * 16 + f"Final Reward = {score}" + marker * 16
    if score == (format_reward + answer_reward):
        acc = True
    else:
        acc = False
