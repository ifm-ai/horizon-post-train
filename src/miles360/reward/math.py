...
def _compute_score_impl(solution_str, ground_truth) -> float:
    retval = 0.0
...
def _compute_score_with_timeout(solution_str, ground_truth) -> float:
    return _compute_score_impl(solution_str, ground_truth)


def compute_score(solution_str, ground_truth, extra_info=None) -> float:
    """The scoring function for MATH-style boxed-answer problems."""
    del extra_info
...
def is_equiv(str1, str2, verbose=False):
    ...
    try:
        ...
        if verbose:
            logger.info("Comparing normalized strings %r and %r", ss1, ss2)
        return ss1 == ss2
    except Exception:
        return str1 == str2
