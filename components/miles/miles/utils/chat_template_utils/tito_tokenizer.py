...
class K2V3TITOTokenizer(TITOTokenizer):
    r"""K2V3 family with the IFM-style chat template used by K2 Horizon.

    The current K2V3 chat template namespaces ChatML tokens as ``<|ifm|im_start|>`` /
    ...
    For legacy K2V3 checkpoints whose chat template uses ``<|im_end|>\n``
...
class K2V3OldBackupTITOTokenizer(TITOTokenizer):
    ...
    Use this with legacy K2V3 checkpoints whose chat template emits
    ``<|im_end|>\n`` after every message (jinja block whitespace between
    ``{{- '<|im_end|>' }}`` and
    ...
    For current K2V3 checkpoints whose template uses ``<|ifm|im_end|>``
    with no trailing ``\n``, use
