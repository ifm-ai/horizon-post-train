...
impl RateLimitSyncAdapter {
    ...
    /// Increment this node's shard in the current one-second UTC window and
    /// return the cluster-wide aggregate for that window.
    ///
    /// The mutex makes local increments linearizable before they are published.
    /// Cross-node visibility remains eventually consistent with mesh gossip,
    /// matching the v1 shared limiter's behavior.
    pub fn increment_and_get_aggregate(&self, counter_name: &str) -> i64 {
        let epoch = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs();
        self.increment_and_get_aggregate_at(counter_name, epoch)
    }

    /// Increment the shared counter and report whether it now exceeds `limit`.
    pub fn check_and_increment(&self, counter_name: &str, limit: u64) -> (bool, i64) {
        let epoch = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs();
        self.check_and_increment_at(counter_name, limit, epoch)
    }

    fn check_and_increment_at(&self, counter_name: &str, limit: u64, epoch: u64) -> (bool, i64) {
        let current_count = self.increment_and_get_aggregate_at(counter_name, epoch);
        let limit = i64::try_from(limit).unwrap_or(i64::MAX);
        (current_count > limit, current_count)
    }

    fn increment_and_get_aggregate_at(&self, counter_name: &str, epoch: u64) -> i64 {
        let count = {
        ...
        self.sync_counter(counter_name, epoch, count);
        self.get_aggregate(counter_name)
    }
...
mod tests {
    ...
    #[test]
    fn increment_tracks_local_window_and_resets_on_epoch_change() {
        let mesh = MeshKV::new("node-a".into());
        let ns = rl_namespace(&mesh);
        let adapter = RateLimitSyncAdapter::new(ns, "node-a".into());

        assert_eq!(adapter.increment_and_get_aggregate_at("global", 10), 1);
        assert_eq!(adapter.increment_and_get_aggregate_at("global", 10), 2);
        assert_eq!(adapter.increment_and_get_aggregate_at("global", 11), 1);
    }

    #[test]
    fn increment_returns_cluster_aggregate_across_node_shards() {
        let mesh = MeshKV::new("node-a".into());
        let ns = rl_namespace(&mesh);
        let node_a = RateLimitSyncAdapter::new(ns.clone(), "node-a".into());
        let node_b = RateLimitSyncAdapter::new(ns, "node-b".into());

        assert_eq!(node_a.increment_and_get_aggregate_at("global", 10), 1);
        assert_eq!(node_b.increment_and_get_aggregate_at("global", 10), 2);
        assert_eq!(node_a.increment_and_get_aggregate_at("global", 10), 3);
    }

    #[test]
    fn shared_limit_allows_limit_and_rejects_next_request() {
        let mesh = MeshKV::new("node-a".into());
        let ns = rl_namespace(&mesh);
        let adapter = RateLimitSyncAdapter::new(ns, "node-a".into());

        assert_eq!(adapter.check_and_increment_at("global", 2, 10), (false, 1));
        assert_eq!(adapter.check_and_increment_at("global", 2, 10), (false, 2));
        assert_eq!(adapter.check_and_increment_at("global", 2, 10), (true, 3));
    }
